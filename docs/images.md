...
This document defines how `klaus` should handle sending and replying with images across both providers. It is intentionally implementation-ready and keeps the UX consistent with existing CLI patterns and Tailscale Funnel usage.

## Goals
- Allow sending an image with an optional caption via `klaus send` for both providers.
...
## CLI & Config Surface
- `klaus send --media <path-or-url> [--message <caption>] [--provider twilio|web]`
...
- Config auto-reply (`~/.klaus/klaus.json`):
...
- When `--media` is a local path, copy to temp dir (`~/.klaus/media/<uuid>`), serve at `/media/<uuid>` on the existing Express app started for webhook, or spin up a short-lived server on demand for `send`.
...
- Fallback: if Funnel is not enabled (or host unknown) and a local path is provided, fail with guidance to run `klaus webhook --ingress tailscale` (or pass a URL instead).
...
- Temp storage: `~/.klaus/media`; cleaned on startup (remove files older than 15 minutes) and during TTL eviction.
...
- Audio/voice notes: if you set `inbound.transcribeAudio.command`, klaus will run that CLI (templated with `{{MediaPath}}`) and replace `Body` with the transcript before continuing the reply flow; verbose logs indicate when transcription runs. The command prompt includes the original media path plus a `Transcript:` section so the model sees both.

## Errors & Messaging
- Local path with twilio + Funnel disabled → error: “Twilio media needs a public URL; start `klaus webhook --ingress tailscale` or pass an https:// URL.”
...
- Should `klaus send --provider twilio --media` implicitly start the media server (even if webhook not running), or require `klaus webhook` already active? (Proposal: auto-start lightweight server on demand, auto-stop after media is fetched or TTL.)
